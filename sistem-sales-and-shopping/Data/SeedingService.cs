...
namespace system_sales_and_shopping.Data
{
    public class SeedingService
    {
        private system_sales_and_shoppingContext _context;

        public SeedingService(system_sales_and_shoppingContext context) //ira receber uma instancia do context também
        {
            _context = context;
        }
        public void Seed()
        {
            if (_context.Department.Any()|| //verificando se há algum dado nas tabelas .any = se não tiver nada
                _context.Seller.Any()||
                _context.SalesRecord.Any())
            {
                return;//o banco ja foi populado
            }
            //instanciaçção dos objetos e inserino no banco de dados
            ...
            _context.SaveChanges(); // salva e confirma as alterações no bando de dados
        }
    }
}
